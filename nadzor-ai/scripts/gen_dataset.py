...
from scripts.dataset import documents as B
...
def build_object_1(out: Path, manifest: list) -> None:
    ...
    p = build_pdf(d / "permit" / "razreshenie.pdf", B.permit_blocks(obj))
    ...
    p = build_pdf(d / "pd_rev2" / "2024-15-KR_izm2.pdf",
                  B.pd_blocks(obj, PD_REV2, "2", "2024-03-12", None),
                  B.stamp_for(obj, "2024-15-КР", "Конструктивные решения", "П", "2", "2024-03-12"),
                  wide=True)
    _doc(manifest, obj["id"], p, "pd", "PD", "Проектная документация, раздел КР, редакция 2",
         "2", "2024-03-12", "КР")

    p = build_pdf(d / "pd_rev3" / "2024-15-KR_izm3.pdf",
                  B.pd_blocks(obj, PD_REV3, "3", "2024-06-26", CHANGES_REV3),
                  B.stamp_for(obj, "2024-15-КР", "Конструктивные решения", "П", "3", "2024-06-26"),
                  wide=True)
    _doc(manifest, obj["id"], p, "pd", "PD", "Проектная документация, раздел КР, редакция 3",
         "3", "2024-06-26", "КР")

    p = build_pdf(d / "expertise" / "zaklyuchenie_expertizy.pdf", B.expertise_blocks(obj), wide=True)
...
def build_small_object(out: Path, manifest: list, obj: dict, acts: list, pd_date: str,
    ...
    p = build_pdf(d / "pd_rev1" / "pd_kr.pdf", B.pd_blocks(obj, PD_REV2[:4], "1", pd_date, None),
                  B.stamp_for(obj, f"{obj['id']}-КР", "Конструктивные решения", "П", "1", pd_date),
                  wide=True)
    _doc(manifest, obj["id"], p, "pd", "PD", "Проектная документация, раздел КР", "1", pd_date, "КР")

    p = build_pdf(d / "rd" / "rd_kzh.pdf", B.rd_blocks(obj, PD_REV2[:4], rd_date),
                  B.stamp_for(obj, f"{obj['id']}-КЖ", "Конструкции железобетонные", "Р", "1", rd_date),
                  wide=True)
    ...
    p = build_pdf(d / "as_built" / "obshchiy_zhurnal_rabot.pdf", B.journal_blocks(obj, acts), wide=True)
