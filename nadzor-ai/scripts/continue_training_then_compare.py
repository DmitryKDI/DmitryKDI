"""Continue training on focused synthetic cases, then rerun real PD/RD transfer."""
...
FOCUS_CORPUS = ROOT / "data" / "synthetic_training" / "focus_v2"
FOCUS_REPORT = ROOT / "data" / "synthetic_training" / "latest_focus_training_report.json"
...
def main() -> int:
    args = parse_args()
    _run([sys.executable, str(SCRIPTS / "generate_focus_corpus.py")])
    _run([
        sys.executable,
    ...
    _run(command)
    print(f"\nFocused training report: {FOCUS_REPORT}")
