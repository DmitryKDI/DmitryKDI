#!/usr/bin/env bash
# Прогон сверки требований (Г.32/Г.33) с эскалацией кандидатов без кода
# системы в зрение по листу РД (Г.33/vision_page_compare.py). В отличие от
# run_no_llm.sh — ТРЕБУЕТ ключ провайдера (по умолчанию GigaChat), читает
# его из отдельного файла ключей, а не из общего .env приложения.
