...
def run_page_pair_comparison(
    ...
    текстом, а сам чертёж отличаться (конфигурация трасс и обвязки и т.п.,
    ...
    for v in verdicts:
        ...
        if v.level == 0:
            ...
            # НА САМОМ ЧЕРТЕЖЕ (конфигурация трасс и обвязки) реестрами не
...
def run_triangulated(
    ...
    try:
        ...
        # (система и её ответвления по расчётной таблице) не участвовала в общем
        ...
        if requirements_llm_config is not None and routing_room_keys:
            ...
            _emit("")
            _emit("=== Сверка «назначено в ПД ↔ нарисовано в РД» "
                  "(Г.58/Г.65, авто по помещениям routing) ===")
            _emit(f"Листов расчётной таблицы найдено: {mo_result.table_pages_found}, "
                  f"помещений с назначенной сущностью: {len(mo_result.pd_entries)}")
            ...
            if mo_result.uncovered:
                _emit("Запрошенные помещения не найдены в расчётной таблице ВООБЩЕ "
            ...
            for room in mo_result.uncovered:
                signals.append(Signal(source="mo_table", domain="room", key=room,
                                       detail=f"пом. {room} не найдено строкой в расчётной таблице вообще (Г.60)"))
...
def _run_mo_cross_check(
    ...
    for path in before_paths:
        ...
        for page in pages:
            ...
            # сущности — любой, какой задан реестром.
    ...
    # "uncovered" осмысленно ТОЛЬКО когда расчётная таблица вообще
...
def run_mo_check(
    ...
    """Сверка «назначено таблицей ПД ↔ нарисовано на листе РД» (Г.58/Г.88).

    Тип таблицы и вид назначаемой сущности берутся из реестра
    (`table_registry`), к разделу механизм не привязан: таблица ПД задаёт
    по помещению систему и её именованные ответвления, лист РД показывает
    те же ответвления нарисованными — иногда у ДРУГОГО помещения или под
    ДРУГИМ обозначением системы.
    ...
    try:
        ...
        _emit("=== Сверка «назначено в ПД ↔ нарисовано в РД» (Г.58) ===")
        _emit(f"Листов расчётной таблицы найдено: {result.table_pages_found}, "
              f"помещений с назначенной сущностью: {len(result.pd_entries)}")
        ...
        if result.uncovered:
            _emit("Запрошенные помещения не найдены в расчётной таблице ВООБЩЕ "
                  "(нет строки — это не то же самое, что пустой столбец, Г.60; "
                  f"нужен другой источник ПД): {', '.join(result.uncovered)}")
        if not result.pd_entries:
            _emit("Ни одного помещения с назначенной сущностью не найдено "
                  "(нет расчётной таблицы в ПД, либо у запрошенных помещений "
                  "сущность не назначается — не путать с молчанием, Г.10)")
            return
        _emit(f"Листов РД просмотрено: {result.candidate_pages_count}, "
              f"ответвлений найдено: {result.rd_branches_count}")
        _emit("")
