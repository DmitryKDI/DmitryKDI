...
_GROUP_HEADER_RE = re.compile(
    r"групп[аы]\s+помещений|\bблок,|зона ожидания", re.I)
# «в том числе» — НЕ признак заголовка группы: в экспликации так помечают
# помещение, у которого следом расписана его же часть («245 Актовый зал на
# 160 мест, в том числе / эстрада в уровне пола»). Отсев по этой фразе молча
# выбрасывал настоящее помещение вместе с его площадью, и оно попадало в
# кандидаты «отсутствует в РД». Хвост отрезаем, помещение оставляем.
_INCLUSIVE_TAIL_RE = re.compile(r"\s*[,:]?\s*в\s+том\s+числе\s*:?\s*$", re.I)
...
def _trim_name(name: str) -> str:
    """Убирает служебный хвост «, в том числе» — он не часть названия."""
    return _INCLUSIVE_TAIL_RE.sub("", name).strip(" ,:")
