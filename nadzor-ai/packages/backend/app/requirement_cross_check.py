...
def _extract_token(sentence: str) -> Optional[str]:
    ...
    экстрактор, который читает фактическое число с чертежа зрением, а не
    ищет совпадение подстроки; такой модуль был построен, но удалён при
    чистке репозитория как ни разу не подключённый к пайплайну, см. Г.62)."""
