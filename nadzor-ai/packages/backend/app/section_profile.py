...
KIND_WORD = "word"      # значимое слово требований тома — накопитель для
                        # различения «слово предмета этого вида томов» и
                        # «слово, называющее конкретное место» (Г.108).
                        # Считается по ТОМАМ: слово, встреченное в одном
                        # томе, — привычка автора; встреченное в нескольких
                        # томах раздела — предмет этого вида документации.
...
def collect_words(texts: Iterable[str]) -> list[str]:
    """Значимые слова требований тома — без частот и без порогов.

    Порог здесь не нужен и вреден: важно не сколько раз слово встретилось
    В ЭТОМ томе (это привычка одного автора), а в скольких РАЗНЫХ томах
    раздела оно встретилось вообще. Второе считает база, и её счётчик
    растёт сам по мере эксплуатации.
    """
    seen: dict[str, str] = {}
    for text in texts:
...
def word_volumes(section: str | None) -> dict[str, int]:
    """Слово → в скольких РАЗНЫХ томах раздела оно встречалось (Г.108).

    Никаких порогов: наружу отдаётся сам счётчик, а как им пользоваться,
    решает потребитель. Счётчик растёт от эксплуатации, поэтому вес слова
    уточняется сам, без правки кода.
    """
    return {e.value: e.documents
            for e in profile(section, kind=KIND_WORD, min_documents=1)}


def topic_words(section: str | None, min_documents: int = HINT_MIN_DOCUMENTS) -> set[str]:
    """Слова, которыми назван ПРЕДМЕТ этого вида томов (Г.108).

    Слово, стоящее в требованиях нескольких разных томов одного раздела, —
    не признак конкретного места, а тема самой документации: оно одинаково
    подходит ко всему тому и потому ничего не различает.

    Пусто, пока база не увидела нужного числа томов, — и это честное
    состояние, а не сбой: различать предмет и место, имея один том, нечем
    (Г.21). Ни одного слова здесь не зашито: список набирается разбором.
    """
    return {e.value for e in profile(section, kind=KIND_WORD, min_documents=min_documents)}
