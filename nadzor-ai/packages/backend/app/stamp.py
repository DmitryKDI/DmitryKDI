...
@dataclass
class Stamp:
    shifr: str | None = None        # хвост шифра: стадия и марка комплекта
...
def read_sheet_name(page: pymupdf.Page) -> str | None:
    ...
    графической части попадало наименование вместе с приклеенным из
    соседней графы полным названием раздела (замер на реальном томе:
    название листа + название раздела одной строкой).
