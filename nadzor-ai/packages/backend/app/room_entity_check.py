...
Г.88 — общий механизм вместо модуля под один раздел. Прямое указание
пользователя: «убери из кода акцент на конкретный пример, код должен быть
общим». Раньше это был `ventilation_mo.py`, написанный целиком под
вентиляцию: заголовок таблицы, названия столбцов, вид обозначений и даже
примеры в промптах были зашиты в код. Само же наблюдение к вентиляции не
привязано:
...
def _table_prompt(kind: TableKind) -> str:
    ...
    Раньше здесь стоял текст про воздухообмены и вытяжные шкафы; теперь
