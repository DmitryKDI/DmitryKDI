...
Г.68: на реальном комплекте 195-176 уникальных формулировок на раздел), вызов на
...
разобранного комплекта, см. докстринг `requirement_registry.py`) — в тексте не
...
GigaChat на 4 разделах реального комплекта (236 кандидатов): фильтр не отсеял НИ
...
дословная цитата с текущего проверяемого объекта (тот же объект, что
и в разборе, Г.11/Г.12 запрещают обучать фильтр на
...
def classify_general_requirements(
    ...
    Г.77 — реальный найденный пробел: сбой ВСЕЙ пачки (например, системная
