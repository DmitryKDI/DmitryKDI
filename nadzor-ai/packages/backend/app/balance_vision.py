...
Как и `balance_box.py` — узкая, ОВ-специфичная реализация (см. оговорку
там), не дисциплино-независимый механизм: `BALANCE_OCR_PROMPT` прямо
называет вентиляцию/отопление, а не принимает `discipline` параметром, как
промпты в `vision.py`/`vision_page_compare.py`/`requirement_llm_extract.py`
— это осознанно, а не забытое подключение: у притока/вытяжки нет
дисциплино-независимой формулировки, придумывать общую версию раньше
второго реального документа другой дисциплины (Г.11) не стоит."""
