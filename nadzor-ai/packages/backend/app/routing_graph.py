...
Замеры сделаны на двух книгах поэтажных планов инженерного раздела реального
комплекта РД: лист 1 (2384×3370 pt, 152 410 путей / 338 113 примитивов) и
лист 2 (164 576 путей). Обозначения ниже названы РОЛЬЮ («подпись ветки»,
«подпись точки сбора»), а не кодами конкретного раздела: замеряемое —
геометрия выноски и цепи, она у любого раздела одна и та же (Г.24).

1. **Подписей веток в текстовом слое НЕТ.** Прогон обеих книг целиком
   (136 страниц) регуляркой `^[А-ЯЁ]{1,2}\\d{1,2}[.,]\\d{1,2}$` по
   `get_text("words")` дал ровно ОДНО попадание на все 136 страниц и ни
   одного вхождения «поз.» на планах. Глазами на рендере того же листа
   подписи веток и точек сбора читаются без труда — они нарисованы кривыми
...
3. **Устройство выноски.** Замер выноски ПОДПИСИ ВЕТКИ: подпись несёт
   СОБСТВЕННЫЙ нарисованный прямоугольник (2093.1, 822.3)–(2113.4, 833.8) с
   диагональю, от угла которого идёт линия длиной 35.4 pt к острию на узле трассы
   (2060.2, 820.6). У ПОДПИСИ ТОЧКИ СБОРА то же самое, но выноска из двух
...
4. **Цепь трассы прослеживается, но НЕ приварена к выноске.** Острие
   выноски ветки не совпадает узлом ни с чем: ближайший узел цепи трассы —
   в 2.26 pt. Сама цепь при этом настоящая и длинная: от узла трассы (x≈2021) до
   символа подключаемого элемента (x≈2256), 138 узлов, и она же несёт
   магистраль к точке сбора. То есть трассировка
   «ветка → точка сбора» физически проходит, но соединение подписи с
   цепью — это ЗАЗОР, а не общий узел.
5. **Символ точки сбора наложен на трассу, а не сварен с ней.** Замер
   при 16-кратном увеличении: оранжевый прямоугольник
   (2236.0, 860.2)–(2266.3, 879.9) с чёрным крестом внутри, синяя
   трасса входит слева и заканчивается пурпурным кружком в центре.
   Остриё выноски попадает в ВЕРХНИЙ ПРАВЫЙ УГОЛ оранжевой рамки,
   а рамка ПЕРЕСЕКАЕТ линии трассы, нигде с ними не совпадая узлами.
...
при этом разрешаются однозначно: подпись ветки → цепь на 0.68 pt, подпись
точки сбора → та же цепь через пересечение рамки символа.

6. **Глобальное увеличение допуска связности вместо этого — тупик.**
   Замер: при snap 0.6 pt цепь трассы = 138 узлов из 10 973 в зоне
...
* **Контуры глифов CAD-текста — тоже пути `get_drawings()`.** Вокруг bbox
  подписи ветки нашлось 39 узлов в 5+ компонентах: штрихи отдельных букв и
  цифр кода (отрезки длиной 4–7.5 pt). Все они касаются bbox подписи и
...
* **Осевые и строительные линии проходят вплотную к символу.** У точки
  сбора ближайший к выноске примитив — одиночный
  ...
  (осевая линия, стрелка выноски, засечка узла трассы) маршрутом не считается.
  Замер разделяет классы с запасом: засечки 2 узла, цепь 138.
* **Стена как крупная компонента рядом.** У острия подписи ветки штриховка стен —
...
* **Полка чужой выноски проходит вплотную к соседней подписи.** Выноска
  СОСЕДНЕЙ ветки — (2060.0, 808.7)–(2116.4, 816.0) — идёт на 6.3 pt выше
  рамки замеряемой. При допуске касания больше ~6 pt она попала бы в её
  декорацию. Поэтому `touch_tolerance` мал (1.5 pt).
...
* **Буква с одной цифрой в экспликации — это графа «Кат. пом.»**
  (категория пожароопасности помещения), а не код системы: на замеренном
  листе таких слов 19 штук, все с y>2600, то есть в таблице. Коллизия
  общая, не разделу свойственная: короткий буквенно-цифровой код в любой
  таблице сталкивается с кодом ветки. Регулярка ветки требует точку и
  цифру после неё, поэтому категория под неё не подходит — но расширять
  её до `^[А-ЯЁ]\\d+$` нельзя, тест это фиксирует.

## Что осталось незакрытым (открытые наблюдения, а не решённые вопросы)

* Если выноска приварена к трассе общим узлом (одна компонента с
...
#
# Шаблоны намеренно записаны ОБЩИМ видом, а не под обозначения одного
# раздела: код ветки — одна-две прописные буквы и номер с уточнением
# («В2.7», «П3.1», «Ст1.2», «К6.4»), точка сбора — позиция с необязательным
# буквенным префиксом типа элемента («М.О. поз.169», «поз.169»). Геометрия
# (`SegmentNetwork`/`trace_label`/`diff_routing_graphs`) дисциплино-независима
# была всегда; распознавание подписей теперь тоже не привязано к разделу.
# `collect_labels()` принимает подписи откуда угодно, так что раздел с другим
# форматом обозначений подключается своим экстрактором подписей, без правки
# геометрии.
BRANCH_CODE_RE = re.compile(r"^[А-ЯЁ]{1,2}\d{1,2}[.,]\d{1,2}$")
# Точка сбора: позиция с необязательным префиксом типа элемента —
# «М.О. поз.169», «Ст. поз.12» либо просто «поз.169»/«поз. 169».
TARGET_CODE_RE = re.compile(r"^(?:[А-ЯЁ]\.?\s?[А-ЯЁ]?\.?\s*)?поз\.?\s*\d{1,4}$", re.I)
...
# Меньше узлов — это осевая линия, стрелка выноски или засечка узла трассы, а не
...
def segment_distance(a1: Point, a2: Point, b1: Point, b2: Point) -> float:
    ...
    рамка символа подключаемого элемента «сходится» с трассой, не имея с ней
    ни одного общего узла (замер, п.5 докстринга)."""
...
def page_segments(page, clip: Bbox | None = None) -> list[Segment]:
    ...
    концов, а не форма дуги — отвод трассы рисуется дугой, и хорда сохраняет
    ту же пару узлов. Прямоугольники и квадры разворачиваются в рёбра, иначе
    символ подключаемого элемента не пересечётся с подходящей к нему трассой."""
...
class SegmentNetwork:
    ...
    чтобы собрать трассу в одну цепь, и не хватает, чтобы слепить лист
    ...
    def decoration_indices(self, bbox: Bbox,
        ...
        обязателен: у подписи точки сбора bbox касается только полки выноски, а
        до трассы дотягивается её дальнее звено вместе с цветной
    ...
    def chains_near(
        ...
        линии, стрелки выносок, засечки узлов трассы (замер в докстринге).
...
def collect_labels(
    ...
    буквенный префикс типа элемента и «поз.NNN» приходят из
    `get_text("words")` ОТДЕЛЬНЫМИ словами, а код точки сбора — это их пара,
    и без жадности одна подпись дала бы сразу две метки (с префиксом и без),
    а ветка увидела бы на своей цепи две точки сбора вместо одной.
...
def build_routing_graph(
    ...
    # законно несколько присоединённых веток (на замеренном листе у одной
    # точки сбора подписаны сразу три ветки), а лишние цепи-кандидаты —
