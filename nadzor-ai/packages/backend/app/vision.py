...
# Реальный сбой: qwen2.5vl:7b на инженерном чертеже (номера помещений,
# оси — мелкий текст) при 1600px не давал конкретики (общие фразы без
# координатного ориентира) — вероятно, подписи физически нечитаемы на
# такой картинке. Поднято; вместе с этим поднят и LOCAL_NUM_CTX в llm.py,
# чтобы более крупная картинка не обрезалась контекстом. Не проверено
# повторно — статус эксперимента, не подтверждённого правила.
VISION_MAX_DIM = 2200
...
# известный источник шума (см. CLAUDE.md Г.28). Проверка одной картинкой,
