...
#
# Г.67 — на реальном комплекте («Школа-600», полная ведомость, за пределами
# узкой ОВ-выборки nadzor_sample) нашлась строка ведомости с обозначением
# СТОРОННЕГО субподрядчика по чужой схеме шифров — не тройка «АНО/150321/1»,
# а пять "/"-сегментов, с пробелом и строчными буквами внутри первого
# сегмента (форма вроде «Орг 25-1-2/РД/ГП/…/…-РД-СКТВ» — сама выдержка не
...
# Г.67 (второй заход) — реальная проверка на «Школа-600» нашла регрессию
# ИМЕННО от этого расширения: PDF переносит длинное обозначение субподрядчика
# посреди строки («...21/695-» / перенос строки / «РД-СКТВ»), и раз "-" сам
...
# усечённый фрагмент «...21/695-» сам по себе стал ПОЛНЫМ валидным
...
# ложный класс, найденный на «Школа-600» — паспортные данные оборудования
