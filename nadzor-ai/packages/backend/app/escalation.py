...
без этого поля рендер кропа по образцу `balance_vision.py` не построить.
Следующий шаг, если понадобится: завести `page` в эти датаклассы (как уже
сделано у `anchor_prose.py`'s hits) и вызывать `render_balance_crop_png`-
подобный рендер здесь. Пока этого нет — пакет честно говорит «страница
неизвестна» вместо того, чтобы притворяться, что кропа не бывает."""
