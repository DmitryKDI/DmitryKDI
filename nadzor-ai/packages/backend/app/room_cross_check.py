...
листов), здесь номер помещения сравнивается напрямую по всему комплекту,
без привязки к какой-то конкретной паре листов (модуль `anchor_pages.py`,
искавший все страницы РД с уже подтверждённым якорем, решал смежную
задачу, но остался неподключённым к пайплайну и был удалён при чистке
репозитория, см. Г.62):
...
def _is_minor_variation(name_a: str, name_b: str) -> bool:
    ...
    Например: «Насосная» → «Насосная №2» — уточнение (добавлен номер).
    Но «Насосная» → «Кладовая» — реальное изменение назначения."""
