...
самом чертеже (например, обвязка трасс внутри помещения с тем же
...
слепой прогон на реальном комплекте: пара с совпавшими реестрами помещений
124/142 пропускалась level=0, хотя обвязка на листе отличалась.
...
def diff_hot_zone(
    ...
    обвязки внутри помещения тонет в общей картинке при сравнении листа
