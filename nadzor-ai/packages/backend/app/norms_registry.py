...
(«СП 60.13330.2020»), а не на пункт внутри него. Пункт нельзя проверить, не
...
требования («трубы стальные водогазопроводные по ГОСТ 3262-75»). Проверяется
поиском подстроки, вызова модели не требует и ошибиться не может.
...
class Norm:
    ...
    перечне стоят в скобках: «СП 60.13330.2020 (СНиП 41-01-2003)». Требование
    в тексте может ссылаться на любое из них, и без псевдонимов ссылка на
    старое обозначение выглядела бы как норматив вне перечня.
    """
    designation: str        # «СП 60.13330.2020»
    title: str = ""         # «Отопление, вентиляция и кондиционирование воздуха»
    page: int = 0
    document: str = ""
...
def _clean_designation(raw: str) -> str:
    ...
    Найдено замером: регулярка захватывала точку в конце фразы («ГОСТ
    10704-91.»), и обозначение переставало сходиться со своей же строкой в
    перечне — норматив, который в перечне ЕСТЬ, попадал в список
    отсутствующих. Тот же класс ошибки, что Г.68: захваченный разделитель
    даёт не честный промах, а неверные данные.
    """
...
def _normalize(designation: str) -> str:
    ...
    «ГОСТ 3262-75*» и «ГОСТ 3262-75» — один документ; звёздочка означает
...
def _quoted_spans(text: str) -> list[tuple[int, int]]:
    ...
    отдельными документами: «СП 131.13330.2020 «СНиП 23-01-99* «Строительная
    климатология»» — это ОДИН документ, а не два."""
...
def parse_norms(text: str, page: int = 0, document: str = "") -> list[Norm]:
    ...
    1. «СП 60.13330.2020 (СНиП 41-01-2003) «Отопление…»» — обозначение в
       СКОБКАХ сразу после основного это тот же документ в прежней системе.
       Наивный разбор заводил из него отдельный норматив И отдавал ему
       наименование, а основной оставался безымянным.
    2. «СП 131.13330.2020 «СНиП 23-01-99* «Строительная климатология»» —
       обозначение внутри КАВЫЧЕК входит в наименование, отдельным
...
def find_norms(text_facts: list[dict]) -> list[Norm]:
    """Перечень нормативов тома. Пустой список означает «раздела перечня в
    документе не найдено» — это видимое состояние, а не повод угадывать
    нормативы по разделу (Г.10)."""
    out: list[Norm] = []
    seen: set[str] = set()
...
def cited_outside_the_list(requirements: list[Requirement],
    ...
    Найдено замером, а не предположено: в томе прямо сказано «краской МА-15
    (ГОСТ 10503-71) по грунту ГФ-021 (ГОСТ 25129-82)», при этом в перечень
    использованных нормативных документов попал только второй. Проектировщик
    сослался на документ, которого сам не объявил.
