...
rem Извлечение требований из ПД ИИ (общий путь — работает на прозе любого
rem раздела/формата, не завязано на один документ, Г.36) со сверкой против
rem РД и ЭСКАЛАЦИЕЙ В ЗРЕНИЕ по листу РД для требований без кода — нужен
rem ключ ИИ-провайдера (по умолчанию GigaChat).
...
echo.
echo ============================================================
echo Готово. Сводка требований ПД и вердикты зрения сохранены в файл:
echo   %HEREW%\requirements_summary.txt
echo (записывались по мере готовности, не только в конце — можно открыть
echo и во время долгого прогона)
echo ============================================================
pause
